...
$(document).ready(function(){
    ...
    $.validator.addMethod("valid_phone", function(value, element){
		...
		var min_length = parseInt($(element).attr('minlength')) || 8;
		if (typeof length !== 'undefined' && (length == 9 || length == 10)){
			if(value.length == 0 || value.length < length){ return false; }
		}else{
			if(value.length == 0 || value.length < min_length || value.length > length){ return false; }
		}
    ...
    $.validator.addMethod("bus_valid_docnum_for_ru", function(value, element){
    ...
    $.validator.addMethod("valid_docnum_for_sirena", function(value, element){
				var _doc = new function() {
					this.sufix = typeof($(element).data('ut')) == 'undefined' ? 'base' : 'ut';

					this.child = function(){
						return /^[IVX]*[А-Яа-я]{2}\d{6}$/.test(value);
					};

					this.adult_base = function(){
						return (value.length >= 9 && value.length <= 10);
					};

					this.adult_ut = function(){
						return (/^\d+$/.test(value) && value.length >= 9 && value.length <= 10 );
					};
				};

				return ( _doc['adult_' + _doc.sufix]() || _doc.child() );
    }, "Please enter a valid document number.");
	...
	$.validator.addMethod("required_check_this_card", function(value, element){
		if(element.checked){
			return true;
		} else {
			return false;
  ...
  ['date_valid', 'date_valid_us'].forEach(function(i){
    $.validator.addMethod(i, function(value, element){
        ...
        if($(element).hasClass("pass")){
	        ...
	        day = $('#' + element.id.replace(replace_element, '_day_')).val();
				...
				if($(element).hasClass('js-ignore-datevalid-if-empty') && year === "" && month === "" && day === "") return true;
    ...
    $.validator.addMethod("valid_cvv2_number", function(value, element){
        var cvv_input = $(element),
            minLength = cvv_input.data('length') && parseInt(cvv_input.data('length')),
    ...
    $.validator.addMethod("valid_card_number_visa_master", function(value, element){
		...
		if(!(first_n >= 2 && first_n <= 6) && (window['front_version'] == 'mobile'  || window['front_version'] == 'v2')){
  ...
  $.validator.addMethod("ru_international", function(value, element){
    ...
    $.validator.addMethod("valid_rgd_loyalty_card_number", function(value, element){
    	var pattern = /^\d{1,13}$/;
      return pattern.test(value);
    }, "Loyalty card must be from 1 to 13 characters.");

    $.validator.addMethod("deny_duplicate_rgd_loyalty_road_card", function(value, element){
    	result = true;
    	var current_loyalty_info_block = $(element).parents('.js_loyalty_info_block');
    	var all_loyalty_info_blocks = $('.js_loyalty_info_block:visible');
    	var condition = current_loyalty_info_block.length && all_loyalty_info_blocks.length > 1;
    	if(condition) {
	    	$.each(all_loyalty_info_blocks, function(i, block) {
	    		block = $(block);
	    		if(block.data('id') != current_loyalty_info_block.data('id')) {
	    			card_type_one = block.find('.js_loyalty_card_type').val();
	    			card_number_one = block.find('.js_loyalty_card_number_input').val();
	    			card_type_two = current_loyalty_info_block.find('.js_loyalty_card_type').val();
	    			card_number_two = current_loyalty_info_block.find('.js_loyalty_card_number_input').val();

	    			deny_duplication = card_type_one === 'RzhdU' && card_type_two === 'RzhdU';
	    			types_cond = card_type_one == card_type_two;
	    			number_cond = card_number_one.length > 0 && card_number_two.length > 0 && card_number_one == card_number_two;
			    	if (deny_duplication && types_cond && number_cond) {
			    		result = false;
			    	}
	    		}
	    	})
      ...
      return result;
    }, "Loyalty road card is duplicated.");
    ...
    $.validator.addMethod("rgd_gender", function(value, element){
        return ($('input[name="' + element.name + '"]:visible:checked').val());
    }, "Please select passenger gender");
